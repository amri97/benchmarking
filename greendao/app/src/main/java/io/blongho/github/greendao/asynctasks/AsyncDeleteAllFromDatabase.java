...
public class AsyncDeleteAllFromDatabase extends AsyncTask<Void, Void, Void> {
  private static final String TAG = "AsyncDeleteAllFromDatab";
  ...
  protected Void doInBackground(Void... voids) {
    timer.addTag("Dropping all customers");
    timer.start();
    daoSession.getCustomerDao().deleteAll();
    timer.stop();
    timer.addTag("Dropping all products");
    timer.start();
    daoSession.getProductDao().deleteAll();
    timer.stop();
    timer.addTag("Dropping all orders");
    timer.start();
    daoSession.getOrderDao().deleteAll();
    timer.stop();
    timer.addTag("Dropping all order products");
    timer.start();
